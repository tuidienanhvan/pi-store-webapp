  ...
  return (
    <div className="stack gap-8">
      <header className="stack gap-2">
        ...
        <p className="m-0 text-18 muted">Fastest channels to get help.</p>
      </header>
      ...
      <section className="stack gap-4">
        <h2 className="m-0 text-24">SLA</h2>
        <Card className="p-0 overflow-hidden">
          <Table>
            ...
            <tbody>
              ...
              <tr><td>Max</td><td className="font-medium text-brand">Direct support</td><td className="font-medium text-brand">4h</td><td><Badge tone="warning">High</Badge></td></tr>
...
function SupportLink({ href, to, icon, title, desc, external = false }) {
  ...
  return (
    <Card as={Comp} {...props} className="stack hover-lift gap-3 p-5 no-underline text-inherit">
      <div className="w-10 h-10 rounded-md bg-surface-2 flex items-center justify-center text-brand">
        <Icon name={icon} size={20} />
      </div>
      <h3 className="m-0 text-18">{title}</h3>
      <p className="m-0 text-14 muted">{desc}</p>
    </Card>
  );
}
