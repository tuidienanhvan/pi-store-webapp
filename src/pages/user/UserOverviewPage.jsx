  ...
  return (
    <div className="stack gap-8">
      <header className="stack gap-2">
        ...
        <p className="m-0 text-18 muted">Pi AI Cloud overview for your current billing cycle.</p>
      </header>
  ...
  return (
    <Card className="p-6">
      <div className="row justify-between items-start flex-wrap gap-3 mb-5">
        <div className="stack gap-1">
          ...
          <div className="row gap-2">
            <h2 className="m-0 text-30">{pkg.package_name}</h2>
            <Badge tone={pkg.status === "active" ? "success" : "warning"}>{pkg.status}</Badge>
          </div>
          <div className="text-13 text-3">Remaining: {daysLeft} days in this cycle</div>
        </div>
      ...
      <div className="stack gap-2">
        <div className="row justify-between text-14 muted">
          <span>Tokens used</span>
          <span>
            <strong className="text-1">{formatNum(used)}</strong>
    ...
    <Card className="p-5">
      <div className="row justify-between items-start">
        <div>
          ...
          <div className="text-28 font-semibold text-1 mt-1">{value}</div>
        </div>
        <Icon name={icon} size={20} className="text-3" />
      </div>
    </Card>
  );
}
...
function UsageChart({ usage }) {
  ...
  if (daily.length === 0) {
    return (
      <Card className="p-6">
        <h3 className="m-0 mb-3 text-18">Usage 30 days</h3>
  ...
  return (
    <Card className="p-6">
      <h3 className="m-0 mb-4 text-18">Usage 30 days</h3>
      <div style={{ display: "flex", gap: 3, alignItems: "flex-end", height: 120 }}>
        {daily.map((d) => (
          <div key={d.date} style={{ flex: 1 }}>
            <div style={{ height: `${Math.max(2, (d.tokens / max) * 100)}%`, background: "var(--brand)", borderRadius: "2px 2px 0 0" }} />
          </div>
        ))}
      </div>
    </Card>
