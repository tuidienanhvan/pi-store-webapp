  ...
  return (
    <section className="flex flex-col gap-6">
      <div className="flex items-center gap-3 px-2">
         ...
         <h2 className="text-[10px] font-semibold tracking-wider text-base-content/40">Tác vụ tự động chạy ngầm (Cron)</h2>
      </div>
      
      {msg && <Alert tone="info" className="rounded-xl border-primary/20 bg-primary/5 text-primary font-semibold text-[11px] tracking-widest">{msg}</Alert>}
      
      <AdminTable>
        <thead>
          <tr className="bg-white/[0.02]">
            <th className="py-4 px-6 text-left font-semibold tracking-wider text-[9px] opacity-40">Tên tác vụ / Mô tả</th>
            <th className="py-4 px-6 text-center font-semibold tracking-wider text-[9px] opacity-40">Lịch chạy</th>
            <th className="py-4 px-6 text-center font-semibold tracking-wider text-[9px] opacity-40">Lần cuối</th>
            ...
            <tr key={j.slug} className="group hover:bg-white/[0.01] transition-all">
              <td className="py-5 px-6">
                <div className="flex flex-col gap-1">
                  <span className="text-[11px] font-bold text-white tracking-tight">{j.name}</span>
                  <span className="text-[9px] font-bold text-base-content/30 tracking-wider">{j.description}</span>
                </div>
              </td>
              ...
              <td className="py-5 px-6 text-center">
                <div className="flex flex-col items-center">
                  <span className="text-[10px] font-bold text-base-content/60">{j.last_run_at ? new Date(j.last_run_at).toLocaleString("vi-VN") : "CHƯA TỪNG CHẠY"}</span>
              ...
              <td className="py-5 px-6 text-right">
                <Button size="sm" variant="ghost" disabled={running === j.slug} onClick={() => executeJob(j.slug)} className="h-9 px-4 rounded-xl border border-white/5 text-[9px] font-semibold tracking-wider bg-white/[0.02] hover:bg-primary/10 hover:border-primary/20">
