...
import { AdminStatCard } from "@admin/pages/components/AdminStatCard";
...
export function AdminKeysPage() {
  ...
  // Bulk release: revoke all keys for a license back to shared pool (T-20260513-001)
  const handleReleaseLicenseKeys = async () => {
    const lid = parseInt(filters.license_id, 10);
    if (!lid || lid <= 0) return;
    if (!confirm(`Thu hồi TẤT CẢ key của license #${lid} về shared pool?\nDùng khi: customer downgrade gói, hủy enterprise, hoặc reset allocation.`)) return;
    try {
      const res = await api.admin.releaseLicenseKeys(lid);
      alert(`Đã release ${res?.released_count ?? 0} key về pool.`);
      load();
    } catch (e) {
      setError(e.message);
    }
          ...
          <header className="flex flex-wrap items-center justify-between gap-6 mb-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Input type="number" placeholder="License ID"
                ...
                className="w-32" />
              {filters.license_id && (
                <Button size="sm" variant="ghost" onClick={handleReleaseLicenseKeys}
                  className="h-9 px-3 rounded-xl border border-warning/40 text-warning font-black uppercase tracking-widest text-[10px]">
                  Thu hồi → Pool
                </Button>
              )}
...
function AddKeyModal({ prefillProviderId, providers, onClose, onSaved }) {
  ...
  return (
    <Modal open={true} onClose={onClose} title="Them provider key" size="md">
      <form onSubmit={submit} className="stack gap-4">
        ...
        <Input
          ...
          placeholder="groq-account-01"
        />
        <Input
          label="Monthly quota tokens (0 = unlimited)"
          type="number"
          min="0"
        ...
        <div className="modal-footer">
          ...
          <Button type="submit" variant="primary" disabled={saving || !form.provider_id || !form.key_value.trim()}>
            {saving ? "Dang luu" : "Luu key"}
          </Button>
        </div>
      </form>
    </Modal>
...
function BulkImportModal({ open, onClose, onSaved, providers }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (!rows.length) throw new Error("Nhap it nhat 1 dong: provider_slug,key,label,quota");
      const res = await api.admin.bulkImportKeys(rows);
      setResult(res);
      await onSaved?.();
    } catch (e2) {
      setErr(e2.message);
  ...
  return (
    <Modal open={true} onClose={onClose} title="Bulk import keys" size="lg">
      <form onSubmit={submit} className="stack gap-4">
        <Alert tone="info">
          Format moi dong: <code>provider_slug,key_value,label,monthly_quota_tokens</code>
        </Alert>
        <Textarea
          rows={12}
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder={`groq-llama-70b-free,gsk_xxx,groq-01,1000000\nopenai-gpt4o-paid,sk_xxx,openai-01,0`}
        />
        <div className="text-[11px] text-base-content/60">
          Providers: {providers.map((p) => p.slug).join(", ")}
        </div>
        {err && <Alert tone="danger">{err}</Alert>}
        {result && (
        ...
        <div className="modal-footer">
          <Button type="button" variant="ghost" onClick={onClose}>Dong</Button>
          <Button type="submit" variant="primary" disabled={saving || !text.trim()}>
            {saving ? "Dang import" : "Import keys"}
          </Button>
        </div>
      </form>
    </Modal>
...
function AllocateModal({ keyObj, onClose, onSaved }) {
  ...
  return (
    <Modal open={true} onClose={onClose} title="Allocate key" size="md">
      <form onSubmit={submit} className="stack gap-4">
        <Card className="stack gap-2">
          <div className="text-[11px] uppercase font-black tracking-widest opacity-50">Key</div>
          <code className="key-cell-code is-masked">{keyObj.key_masked}</code>
          <div className="text-xs opacity-60">{keyObj.provider_slug}</div>
        </Card>
        <Input
          label="License ID"
          required
        ...
        <div className="modal-footer">
          <Button type="button" variant="ghost" onClick={onClose}>Huy</Button>
          <Button type="submit" variant="primary" disabled={saving || !licenseId}>
            {saving ? "Dang cap phat" : "Cap phat"}
          </Button>
        </div>
      </form>
    </Modal>
